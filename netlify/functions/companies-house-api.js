...
exports.handler = async function(event, context) {
  ...
  if (!apiKey) {
    return {
      statusCode: 401,
      body: JSON.stringify({ error: "Companies House API key is missing on the server" })
    };
  }
    ...
    if (isCompanyNumber) {
      ...
      if (profileRes.ok) {
        ...
        const record = {
          ...
          directors: directors,
          personsWithSignificantControl: [], // PSC requires another call, we can leave empty or query
